...
void initScene() {
    spheres[0] = Sphere(vec3(-0.8, 0.0, -3.0), 0.5, vec3(1, 0.2, 0.2), 0.5, 0.0, 1.0);   // espejo rojo
    spheres[1] = Sphere(vec3(0.8, 0.0, -3.2), 0.5, vec3(0.2, 0.8, 1), 0.3, 0.0, 1.0);     // azul metálico
    spheres[2] = Sphere(vec3(0.0, -1001.0, -3.0), 1000.0, vec3(1), 0.0, 0.0, 1.0);         // plano
    spheres[3] = Sphere(vec3(0.0, 0.5, -5.0), 0.4, vec3(1.0, 1.0, 0.6), 0.05, 0.9, 1.5);   // vidrio
}

vec3 rayDirection(vec2 uv) {
    uv *= vec2(1.0, 600.0 / 800.0); // aspecto
...
void main() {
    initScene();
    vec3 ro = vec3(0.0, 0.0, 2.0);        // origen de la cámara
    vec3 rd = rayDirection(fragCoord);   // dirección del rayo
    vec3 color = vec3(0.0);
    vec3 mask = vec3(1.0);               // acumulador de color
    ...
    for (int bounce = 0; bounce < MAX_BOUNCES; ++bounce) {
        ...
        if (s.transparency > 0.0) {
            newRo = hit - n * EPSILON;
            newRd = refrDir;
            mask *= mix(vec3(1.0), baseColor, 0.3); // atenuación por color
